...
def update_task():
    global tasks
    task_number = request.json.get('task_number', None)
    new_task = request.json.get('new_task', None)
    if task_number is not None and task_number.isdigit() and new_task is not None:
        task_number = int(task_number)
        if 1 <= task_number <= len(tasks):
            tasks[task_number - 1] = new_task
            message = f"Updated task number {task_number} to: {new_task}"
        else:
            message = "Invalid task number"
    else:
